...
-  Unexpected "~"character in "Mass (g)" column
...
data_CochiseCoGila <- data_CochiseCoGila %>%
  ...
  mutate(
    ...
    month_cat = factor(month(date, label = TRUE, abbr = TRUE)), # make month an ordered factor
    mass = str_remove_all(mass, "[^0-9\\.]+"),   # remove everything except digits and decimal points
    mass = as.numeric(mass)                       # convert to numeric
  )
